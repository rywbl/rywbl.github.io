...
var IMUtil = {
	...
	//去除非打印字符
	clearUnprintChars: function(str) {
        return str.replace(/[\x00-\x1f\x7f-\x9f]/g, function (x) {
            return "";
        })
	},
	//给网址包装超链接
	...
	hasTopDialog: function(){
		...
		var isPcRelinkShow = $('.pc-recon').is(':visible');
		return isExistTopDg || isSlideDivShow || isHrmCardDivShow || isPcRelinkShow;
	},
	// 处理编码为160的空格
...
jQuery.fn.extend({
	autoHideArray:{},
    debounceAction: function(time, action){
	    var  obj = this;
	    var $obj = $(obj);
        var actionlock = !!$obj.data('actionlock');
        if (actionlock) {
            return false;
        } else {
            $obj.data('actionlock', true);
            setTimeout(function(){
                $obj.data('actionlock', false);
            }, time);
            action && action();
        }
    },
