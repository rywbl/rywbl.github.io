  ...
  var settings = {}, roots = {}, caches = {},
    ...
    view = {
      ...
      appendNodes: function (setting, level, nodes, parentNode, index, initFlag, openFlag) {
        ...
        for (var i = 0, l = nodes.length; i < l; i++) {
          ...
          if (openFlag) {
            ...
            if (setting.weaProps && setting.weaProps.browserTree) {
              var wrapClass = data.isSelectedNode(setting, node)?'wea-tree-wrap wea-tree-wrap-true':'wea-tree-wrap';
              html.push('<div class="',wrapClass,'" style="margin:0;width:100%;lineHeight:26px;padding:0 10px 0 ', node.level * 18 + 10 ,'px">');
            }
      ...
      cancelPreSelectedNode: function (setting, node, excludeNode) {
        ...
        for (i = list.length - 1; i >= 0; i--) {
          ...
          if (node === n || (!node && (!excludeNode || excludeNode !== n))) {
            ...
            $$(n, consts.id, setting).removeClass(consts.node.CURSELECTED);
            if (node) {
      ...
      makeNodeIcoClass: function (setting, node) {
        ...
        return (node.iconSkin ? node.iconSkin : "");
      },
      ...
      makeNodeLineClass: function (setting, node) {
        ...
        if (setting.view.showLine) {
          ...
          if (data.nodeIsParent(setting, node)) {
            if (node.open) {
              return 'icon-coms-organization-down placeholder';
            } else {
              return 'icon-coms-organization-right placeholder';
            }
          }
        }
        if (data.nodeIsParent(setting, node)) {
      ...
      replaceSwitchClass: function (node, obj, newName) {
        ...
        switch (newName) {
          ...
          case consts.line.NOLINE:
            ...
            break;
          case consts.folder.OPEN:
            tmpList[0] = 'icon-coms-organization-down placeholder';
            tmpList[1] = '';
            break;
          case consts.folder.CLOSE:
            tmpList[0] = 'icon-coms-organization-right placeholder';
            tmpList[1] = '';
            break;
          case consts.folder.DOCU:
            tmpList[1] = newName;
      ...
      selectNode: function (setting, node, addFlag) {
        ...
        $$(node, consts.id, setting).addClass(consts.node.CURSELECTED);
