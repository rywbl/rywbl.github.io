...
var IMAGESPATH = window.ecologyContentPath+'/wui/theme/ecology8/skins/default/rightbox/'; //图片路径配置
...
var confirmJson = {
	...
	"okLabel_8": "Sure",
	"cancelLabel_7": "取消",
	"cancelLabel_8": "Cancel",
	"title_7": "信息确认",
	"title_8": "Information Validation"
}
var langId = window.localStorage.getItem("login") && JSON.parse(window.localStorage.getItem("login")).langId || '7';
var Dialog = function (currentwin) {
    ...
    this.okButton = null;
	this.e8SepLine = null;
    this.okLabel = (langId == '7' ? confirmJson.okLabel_7 : confirmJson.okLabel_8);
    this.cancelLabel = (langId == '7' ? confirmJson.cancelLabel_7 : confirmJson.cancelLabel_8);
  ...
  <table id="_DialogTable_' + this.ID + '"  width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0"  style=" font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "")+(this.hideDraghandle ? "display:none;" : "") + '" class="' + this.dialogtitleclass + '">\
      <td height="30px;" style="background-color:#4f81bd;"><div style="float: left;color:#fff;"><span style="width:10px;display:inline-block;"></span><span id="_Title_' + this.ID + '" style="font-size:13px;height:30px;line-height:30px;"> ' +  this.Title  + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url('+window.ecologyContentPath+'/images/ecology8/16_close_wev8.png)\'" onmouseover="this.style.backgroundImage=\'url('+window.ecologyContentPath+'/images/ecology8/16_close2_wev8.png)\'" style="font-family:Verdana!important;\
        				margin: 10px 8px 0; font-size:8px!important;position: relative; cursor: pointer; float: right; height: 16px; width: 16px;background-image: url('+window.ecologyContentPath+'/images/ecology8/16_close_wev8.png);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div>';
        if(this.maxiumnable){
			html += '\
		...
		margin: 10px 8px 0; font-size:8px!important;position: relative; cursor: pointer; float: right; height: 16px; width: 16px;background-image: url('+window.ecologyContentPath+'/images/ecology8/max_wev8.png);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div>';
		}
		html += '</td>\
    </tr>\
    <tr valign="top">\
...
Dialog.prototype.setLoadDiv = function () {
   ...
   $loadImg = $("<img />");
   $loadImg.attr("src", window.ecologyContentPath+"/rdeploy/crm/image/loading.gif");
...
Dialog.prototype.mouseOut = function(obj){
	if(this.maxState){//最大化状态
		jQuery(obj).css("background-image","url("+window.ecologyContentPath+"/images/ecology8/recovery_wev8.png)");
	}else{
		jQuery(obj).css("background-image","url("+window.ecologyContentPath+"/images/ecology8/max_wev8.png)");
	}
}

Dialog.prototype.mouseOver = function(obj){
	if(this.maxState){//最大化状态
		jQuery(obj).css("background-image","url("+window.ecologyContentPath+"/images/ecology8/recovery2_wev8.png)");
	}else{
		jQuery(obj).css("background-image","url("+window.ecologyContentPath+"/images/ecology8/max2_wev8.png)");
	}
}
	    ...
	    if (ids!= ""){
			...
			if(isAdd){
				...
				if(isAdd){
					...
					if(isMustInput==2){
						e8os.find("#"+fieldid+"spanimg").html("<img align='absmiddle' src='"+window.ecologyContentPath+"/images/BacoError_wev8.gif'>");
					}
				}
		}
		if(!!userCallBack){
...
Dialog.prototype.show = function () {
    ...
    this.zindex = thisdialogDiv.style.zIndex = parseInt(this.bgDiv.style.zIndex);
    if (topWin.Dialog._Array.length > 0) {
        this.zindex = thisdialogDiv.style.zIndex = parseInt(topWin.Dialog._Array[topWin.Dialog._Array.length - 1].zindex);
    } else {
    ...
    if (this.Modal) {
        bgdiv.style.zIndex = parseInt(topWin.Dialog._Array[topWin.Dialog._Array.length - 1].zindex);
        Dialog.setBgDivSize();
...
Dialog.prototype.closeManual = function(dialog){
    ...
    if (topWin.Dialog._Array.length > 0) {
        ...
        this.getBgdiv().style.zIndex = "1050";
		this.getBgdiv().style.display = "none";
        var topWinBody = topDoc.getElementsByTagName(topDoc.compatMode == "BackCompat" ? "BODY" : "HTML")[0];
    ...
    if (isInternetExplorer) {
        ...
        try{
			...
			// if (dialog.isIframe) {
			// 	dialog.innerFrame.src = '';
			// 	dialog.innerFrame.contentWindow.close(); //避免frame内存泄漏
			// 	dialog.innerFrame.parentNode.removeChild(dialog.innerFrame);
			// }
        }catch(e){
	...
	jQuery("#_DialogBGDiv_"+dialog.ID, window.top.document).remove();
	this.bgDiv = null;
    dialog.closed = true;
...
Dialog.getInstance = function (id) {
    ...
    if (!dialogDiv){
    	if(window.console)
    	console.log(window.ecologyContentPath+"/wui/theme/ecology8/jquery/js/zDialog.js-->getInstance-->没有取到对应ID的弹出框页面对象");
    }
	try{
...
Dialog.prototype.getBgdiv = function () {
    if (this.bgDiv) {
    	jQuery(this.bgDiv).unbind("contextmenu").bind("contextmenu",function(e){
    		e.preventDefault();
    		e.stopPropagation();
    		return false;
    	});
    ...
    var bgdiv = topWin.$id("_DialogBGDiv_" + this.ID);
    if (!bgdiv) {
        bgdiv = topDoc.createElement("div");
        bgdiv.id = "_DialogBGDiv_" + this.ID;
        bgdiv.style.cssText = "position:absolute;left:0px;top:0px;width:100%;height:100%;z-index:1050";
...
Dialog.close = function (id) {
	var canClose = false;
	var lastE8DialogIframe = $('body>iframe[id*="e8shadowiframe"]').last()[0];
	var isLastElm = lastE8DialogIframe === document.body.lastElementChild;
	if (!isLastElm) {
		var testElm = document.body.lastElementChild;
		while(lastE8DialogIframe !== testElm) {
			if ($(testElm).find('.ant-modal-wrap').length > 0) {
				if ($(testElm).find('.ant-modal-wrap')[0].style.display !== 'none') {
					break;
				}
			}
			testElm = testElm.previousElementSibling;
			if (lastE8DialogIframe === testElm) {
				canClose = true
			}
		}
	} else {
    ...
    if (topWin.Dialog._Array.length > 0 && canClose) {
...
Dialog.alert = function (msg, func, w, h,m,_options) {
    ...
    if(options.isIframe){
    	diag.URL=window.ecologyContentPath+"/systeminfo/alert.jsp?id="+this.ID;
    }else{
    ...
    diag.cancelButton.value = (langId == '7' ? confirmJson.okLabel_7 : confirmJson.okLabel_8);
...
Dialog.confirm = function (msg, funcOK, funcCal, w, h,m, custombtnjson, btnoklabel, btncancellabel, win, flashs) {
	...
	diag.ShowButtonRow = true;
	diag.Title = (langId == '7' ? confirmJson.title_7 : confirmJson.title_8);
    diag.Modal = m;
    diag.CancelEvent = function () {
        diag.close();
        if (funcCal) {
...
function setBtnHoverClass(){
	...
	$(document).on('mousedown', '.zd_btn_submit,.zd_btn_submit2,.zd_btn_cancle', function () {
		jQuery(this).addClass("e8_btn_mousedown");
	}).on('mouseup', '.zd_btn_submit,.zd_btn_submit2,.zd_btn_cancle', function () {
